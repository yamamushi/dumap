...
TexyObject
{
    ...
    __callStatic($name, $args)
    {
        ...
        throw
        new
        MemberAccessException("Call to undefined static method $class::$name().");
    }
    ...
    function&__get($name)
    {
        ...
        if ($name === '') {
            throw
            new
            MemberAccessException("Cannot read a class '$class' property without name.");
        }
        $name[0] = $name[0] & "\xDF";
        $m = 'get' . $name;
        ...
        throw
        new
        MemberAccessException("Cannot read an undeclared property $class::\$$name.");
    }
    ...
    __set($name, $value)
    {
        ...
        if ($name === '') {
            throw
            new
            MemberAccessException("Cannot assign to a class '$class' property without name.");
        }
        $name[0] = $name[0] & "\xDF";
        if (self::hasAccessor($class, 'get' . $name) || self::hasAccessor($class, 'is' . $name)) {
            ...
            if (self::hasAccessor($class, $m)) {
                ...
                throw
                new
                MemberAccessException("Cannot assign to a read-only property $class::\$$name.");
            }
        }
        $name = func_get_arg(0);
        throw
        new
        MemberAccessException("Cannot assign to an undeclared property $class::\$$name.");
    }
    ...
    __unset($name)
    {
        ...
        throw
        new
        MemberAccessException("Cannot unset the property $class::\$$name.");
    }
    ...
    ArrayAccess, IteratorAggregate
{
    ...
    static $xhtml = TRUE;
    public
    static $emptyElements = array('img' => 1, 'hr' => 1, 'br' => 1, 'input' => 1, 'meta' => 1, 'area' => 1, 'base' => 1, 'col' => 1, 'link' => 1, 'param' => 1, 'basefont' => 1, 'frame' => 1, 'isindex' => 1, 'wbr' => 1, 'embed' => 1);
    public
    ...
    setName($name, $empty = NULL)
    {
        if ($name !== NULL && !is_string($name)) {
            throw
            new
            InvalidArgumentException("Name must be string or NULL.");
        }
        $this->name = $name;
    ...
    setText($text)
    {
        if (is_scalar($text)) {
            ...
            throw
            new
            InvalidArgumentException('Content must be scalar.');
        }
        return $this;
    }
    ...
    startTag()
    {
        ...
        if (is_array($this->attrs)) {
            foreach ($this->attrs
                ...
                if ($value === TRUE) {
                    if (self::$xhtml) $s .= ' ' . $key . '="' . $key . '"'; else$s .= ' ' . $key;
                    continue;
                    ...
                    foreach ($value
                             as $k => $v) {
                        if ($v == NULL) continue;
                        if (is_string($k)) $tmp[] = $k . ':' . $v; else$tmp[] = $v;
                    }
                    if (!$tmp) continue;
    ...
    getContentType()
    {
        if (!isset(self::$inlineElements[$this->name])) return
            Texy::CONTENT_BLOCK;
        return
            self::$inlineElements[$this->name] ? Texy::CONTENT_REPLACED : Texy::CONTENT_MARKUP;
    }
    ...
    validateChild($child, $dtd)
    {
        if (isset($dtd[$this->name])) {
            if ($child
                instanceof
                TexyHtml) $child = $child->name;
            return
                isset($dtd[$this->name][1][$child]);
        } else {
            return
                TRUE;
        }
    ...
    {
        $parser = new
        TexyBlockParser($texy, $this, $indented);
        $parser->parse($s);
    }
}
        ...
        while ($p < $len) {
            ...
            if ($ch === '(') {
                ...
                foreach (explode(';', substr($mod, $p + 1, $a - $p - 2)) as $value) {
                    ...
                    $value = trim($pair[1]);
                    if (isset(self::$elAttrs[$prop])) $this->attrs[$prop] = $value; elseif ($value !== '') $this->styles[$prop] = $value;
                }
                $p = $a;
            } elseif ($ch === '[') {
                $a = strpos($mod, ']', $p) + 1;
                ...
                foreach (explode(' ', $s) as $value) {
                    if ($value === '') continue;
                    if ($value{0} === '#') $this->id = substr($value, 1); else$this->classes[$value] = TRUE;
                }
                $p = $a;
            } elseif ($ch === '^') {
                $this->vAlign = 'top';
    ...
    decorate($texy, $el)
    {
        ...
        if (!$this->attrs) {
            ...
            if ($tmp === Texy::ALL) {
                ...
                $tmp = array_flip($tmp);
                foreach ($this->attrs
                         as $key => $value) if (isset($tmp[$key])) $el->attrs[$key] = $value;
            }
            $el->validateAttrs($texy->dtd);
        }
        ...
        if ($this->classes || $this->id !== NULL) {
            ...
            if ($tmp === Texy::ALL) {
                foreach ($this->classes
                         as $value => $foo) $elAttrs['class'][] = $value;
                $elAttrs['id'] = $this->id;
            } elseif (is_array($tmp)) {
                foreach ($this->classes
                         as $value => $foo) if (isset($tmp[$value])) $elAttrs['class'][] = $value;
        ...
        if ($this->styles) {
            ...
            if ($tmp === Texy::ALL) {
                foreach ($this->styles
                         as $prop => $value) $elAttrs['style'][$prop] = $value;
            } elseif (is_array($tmp)) {
                foreach ($this->styles
                         as $prop => $value) if (isset($tmp[$prop])) $elAttrs['style'][$prop] = $value;
            }
        }
        if ($this->hAlign) {
    ...
    TexyParser
{
    ...
    public
    function
    __construct(Texy $texy, TexyHtml $element, $indented)
    {
        $this->texy = $texy;
    ...
    next($pattern, &$matches)
    {
        ...
        if ($ok) {
            ...
            foreach ($matches
                     as $key => $value) $matches[$key] = $value[0];
        }
        return $ok;
    }
    ...
    {
        while (--$this->offset > 0) if ($this->text{$this->offset - 1} === "\n") {
            $linesCount--;
    ...
    parse($text)
    {
        ...
        $matches[] = array(strlen($text), NULL, NULL);
        $el = $this->element;
        $cursor = 0;
        do {
            do {
            ...
            $res = call_user_func_array($this->patterns[$mName]['handler'], array($this, $mMatches, $mName));
    ...
    TexyParser
{
    ...
    public
    function
    __construct(Texy $texy, TexyHtml $element)
    {
        $this->texy = $texy;
        $this->element = $element;
    ...
    parse($text)
    {
        ...
        do {
            ...
            foreach ($names
                     as $name) {
                if ($arrOffset[$name] < $start + $len) $arrOffset[$name] = -1; else$arrOffset[$name] += $delta;
            }
            if ($this->again) {
                $arrOffset[$min] = -2;
            } else {
...
TexyUtf
{
    ...
    {
        return
            iconv('utf-8', $encoding . '//TRANSLIT', $s);
    }
    ...
    strtolower($s)
    {
        if (function_exists('mb_strtolower')) return
    ...
    utf2html($s, $encoding)
    {
        ...
        return
            preg_replace_callback('#[\x80-\x{FFFF}]#u', array(__CLASS__, 'cb'), $s);
    }
...
{
    public
    static $safeTags = array('a' => array('href', 'title'), 'acronym' => array('title'), 'b' => array(), 'br' => array(), 'cite' => array(), 'code' => array(), 'em' => array(), 'i' => array(), 'strong' => array(), 'sub' => array(), 'sup' => array(), 'q' => array(), 'small' => array());
    ...
    TexyObject
{
    ...
    public
    function
    __construct($handlers, TexyParser $parser, $args)
    {
    ...
    proceed()
    {
        if ($this->pos === 0) {
            throw
            new
            InvalidStateException('No more handlers.');
        }
        if (func_num_args()) {
        ...
        if ($res === NULL) {
            throw
            new
            UnexpectedValueException("Invalid value returned from handler '" . print_r($this->handlers[$this->pos], TRUE) . "'.");
        }
        return $res;
    }
    ...
    TexyModule
{
    ...
    __construct($texy)
    {
        ...
        $texy->registerBlockPattern(array($this, 'pattern'), '#^/--++ *+(.*)' . TEXY_MODIFIER_H . '?$((?:\n(?0)|\n.*+)*)(?:\n\\\\--.*$|\z)#mUi', 'blocks');
    }
    ...
    solve($invocation, $blocktype, $s, $param, $mod)
    {
        ...
        if (empty($tx->allowed[$blocktype])) return
            FALSE;
        if ($blocktype === 'block/texysource') {
            $s = Texy::outdent($s);
            if ($s === '') return "\n";
            $el = TexyHtml::el();
            if ($param === 'line') $el->parseLine($tx, $s); else$el->parseBlock($tx, $s);
            $s = $el->toHtml($tx);
    ...
    TexyModule
{
    ...
    __construct($texy)
    {
        ...
        $texy->registerBlockPattern(array($this, 'patternUnderline'), '#^(\S.*)' . TEXY_MODIFIER_H . '?\n' . '(\#{3,}|\*{3,}|={3,}|-{3,})$#mU', 'heading/underlined');
        $texy->registerBlockPattern(array($this, 'patternSurround'), '#^(\#{2,}+|={2,}+)(.+)' . TEXY_MODIFIER_H . '?()$#mU', 'heading/surrounded');
    }
...
TexyHorizLineModule
    ...
    TexyModule
{
    public $classes = array('-' => NULL, '*' => NULL);
    ...
    __construct($texy)
    {
        ...
        $texy->registerBlockPattern(array($this, 'pattern'), '#^(\*{3,}|-{3,})\ *' . TEXY_MODIFIER . '?()$#mU', 'horizline');
    }
    ...
    public
    function
    solve($invocation, $type, $modifier)
    {
        $el = TexyHtml::el('hr');
    ...
    TexyModule
{
    ...
    __construct($texy)
    {
        ...
        $texy->registerLinePattern(array($this, 'patternTag'), '#<(/?)([a-z][a-z0-9_:-]*)((?:\s+[a-z0-9:-]+|=\s*"[^"' . TEXY_MARK . ']*"|=\s*\'[^\'' . TEXY_MARK . ']*\'|=[^\s>' . TEXY_MARK . ']+)*)\s*(/?)>#isu', 'html/tag');
        $texy->registerLinePattern(array($this, 'patternComment'), '#<!--([^' . TEXY_MARK . ']*?)-->#is', 'html/comment');
    }
    ...
    patternTag($parser, $matches)
    {
        ...
        if ($isStart) {
            ...
            foreach ($matches2
                ...
                $key = strtolower($m[1]);
                $value = $m[2];
                if ($value == NULL) $el->attrs[$key] = TRUE; elseif ($value{0} === '\'' || $value{0} === '"') $el->attrs[$key] = Texy::unescapeHtml(substr($value, 1, -1));
    ...
    solveTag($invocation, TexyHtml $el, $isStart, $forceEmpty = NULL)
    {
        ...
        if (is_array($allowedTags)) {
            if (!isset($allowedTags[$name])) return
                FALSE;
        ...
        if (!$allowedAttrs) {
            ...
            foreach ($elAttrs
                     as $key => $foo) if (!isset($allowedAttrs[$key])) unset($elAttrs[$key]);
        }
        $tmp = $tx->_classes;
        if (isset($elAttrs['class'])) {
            if (is_array($tmp)) {
                $elAttrs['class'] = explode(' ', $elAttrs['class']);
                foreach ($elAttrs['class'] as $key => $value) if (!isset($tmp[$value])) unset($elAttrs['class'][$key]);
            } elseif ($tmp !== Texy::ALL) {
                $elAttrs['class'] = NULL;
            }
        }
        ...
        if ($name === 'img') {
            if (!isset($elAttrs['src'])) return
                FALSE;
            if (!$tx->checkURL($elAttrs['src'], Texy::FILTER_IMAGE)) return
                FALSE;
            $tx->summary['images'][] = $elAttrs['src'];
        } elseif ($name === 'a') {
            if (!isset($elAttrs['href']) && !isset($elAttrs['name']) && !isset($elAttrs['id'])) return
                FALSE;
            if (isset($elAttrs['href'])) {
                ...
                if (!$tx->checkURL($elAttrs['href'], Texy::FILTER_ANCHOR)) return
                    FALSE;
    ...
    TexyModule
{
    ...
    __construct($texy)
    {
        ...
        $texy->registerBlockPattern(array($this, 'pattern'), '#^' . TEXY_IMAGE . TEXY_LINK_N . '?? +\*\*\* +(.*)' . TEXY_MODIFIER_H . '?()$#mUu', 'figure');
    }
    ...
    pattern($parser, $matches)
    {
        ...
        if ($mLink) {
            if ($mLink === ':') {
                $link = new
                TexyLink($image->linkedURL === NULL ? $image->URL : $image->linkedURL);
                $link->raw = ':';
    ...
    public
    function
    solve($invocation, TexyImage $image, $link, $content, $mod)
    {
        $tx = $this->texy;
    ...
    TexyModule
{
    ...
    __construct($texy)
    {
        ...
        $texy->addHandler('beforeParse', array($this, 'beforeParse'));
        $texy->registerLinePattern(array($this, 'patternImage'), '#' . TEXY_IMAGE . TEXY_LINK_N . '??()#Uu', 'image');
    }
    ...
    beforeParse($texy, &$text)
    {
        if (!empty($texy->allowed['image/definition'])) {
            $text = preg_replace_callback('#^\[\*([^\n]+)\*\]:\ +(.+)\ *' . TEXY_MODIFIER . '?\s*()$#mUu', array($this, 'patternReferenceDef'), $text);
        }
    }
    ...
    getReference($name)
    {
        ...
        if (isset($this->references[$name])) return
            clone$this->references[$name];
        return
            FALSE;
    }
    ...
    solve($invocation, TexyImage $image, $link)
    {
        ...
        if (!isset($el->attrs['alt'])) {
            if ($alt !== NULL) $el->attrs['alt'] = $tx->typographyModule->postLine($alt); else$el->attrs['alt'] = $this->defaultAlt;
        }
        if ($hAlign) {
            $var = $hAlign . 'Class';
            if (!empty($this->$var)) {
    ...
    TexyModule
{
    ...
    __construct($texy)
    {
        ...
        $texy->addHandler('beforeParse', array($this, 'beforeParse'));
        $texy->registerLinePattern(array($this, 'patternReference'), '#(\[[^\[\]\*\n' . TEXY_MARK . ']+\])#U', 'link/reference');
        $texy->registerLinePattern(array($this, 'patternUrlEmail'), '#(?<=^|[\s([<:\x17])(?:https?://|www\.|ftp://)[0-9.' . TEXY_CHAR . '-][/\d' . TEXY_CHAR . '+\.~%&?@=_:;\#,\x{ad}-]+[/\d' . TEXY_CHAR . '+~%?@=_\#]#u', 'link/url', '#(?:https?://|www\.|ftp://)#u');
        $texy->registerLinePattern(array($this, 'patternUrlEmail'), '#(?<=^|[\s([<:\x17])' . TEXY_EMAIL . '#u', 'link/email', '#' . TEXY_EMAIL . '#u');
    }
    ...
    patternUrlEmail($parser, $matches, $name)
    {
        ...
        TexyLink($mURL);
        $this->checkLink($link);
        return $this->texy->invokeAroundHandlers($name === 'link/email' ? 'linkEmail' : 'linkURL', $parser, array($link));
    }
...
TexyListModule
    ...
    TexyModule
{
    public $bullets = array('*' => array('\*\ ', 0, ''), '-' => array('[\x{2013}-](?![>-])', 0, ''), '+' => array('\+\ ', 0, ''), '1.' => array('1\.\ ', 1, '', '\d{1,3}\.\ '), '1)' => array('\d{1,3}\)\ ', 1, ''), 'I.' => array('I\.\ ', 1, 'upper-roman', '[IVX]{1,4}\.\ '), 'I)' => array('[IVX]+\)\ ', 1, 'upper-roman'), 'a)' => array('[a-z]\)\ ', 1, 'lower-alpha'), 'A)' => array('[A-Z]\)\ ', 1, 'upper-alpha'));
    ...
    {
        $RE = $REul = array();
        foreach ($this->bullets
                 as $desc) {
            $RE[] = $desc[0];
            if (!$desc[1]) $REul[] = $desc[0];
        }
        $this->texy->registerBlockPattern(array($this, 'patternList'), '#^(?:' . TEXY_MODIFIER_H . '\n)?' . '(' . implode('|', $RE) . ')\ *\S.*$#mUu', 'list');
        $this->texy->registerBlockPattern(array($this, 'patternDefList'), '#^(?:' . TEXY_MODIFIER_H . '\n)?' . '(\S.*)\:\ *' . TEXY_MODIFIER_H . '?\n' . '(\ ++)(' . implode('|', $REul) . ')\ *\S.*$#mUu', 'list/definition');
    }
    ...
    patternList($parser, $matches)
    {
        ...
        $bullet = $min = NULL;
        foreach ($this->bullets
                 as $type => $desc) if (preg_match('#' . $desc[0] . '#Au', $mBullet)) {
            $bullet = isset($desc[3]) ? $desc[3] : $desc[0];
            $min = isset($desc[3]) ? 2 : 1;
            $el->setName($desc[1] ? 'ol' : 'ul');
            $el->attrs['style']['list-style-type'] = $desc[2];
            if ($desc[1]) {
                if ($type[0] === '1' && (int)$mBullet > 1) $el->attrs['start'] = (int)$mBullet; elseif ($type[0] === 'a' && $mBullet[0] > 'a') $el->attrs['start'] = ord($mBullet[0]) - 96;
    ...
    patternDefList($parser, $matches)
    {
        ...
        $bullet = NULL;
        foreach ($this->bullets
                 as $desc) if (preg_match('#' . $desc[0] . '#Au', $mBullet)) {
            $bullet = isset($desc[3]) ? $desc[3] : $desc[0];
            break;
        }
        $el = TexyHtml::el('dl');
    ...
    patternItem($parser, $bullet, $indented, $tag)
    {
        ...
        $matches = NULL;
        if (!$parser->next($patternItem, $matches)) return
            FALSE;
        ...
        if (isset($elItem[0]) && $elItem[0] instanceof
            TexyHtml) {
    ...
    TexyModule
{
    public $wordLimit = 20;
    const
        DONT = 0, HERE = 1, AFTER = 2;
    private $consonants = array('b', 'c', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'p', 'q', 'r', 's', 't', 'v', 'w', 'x', 'z', 'B', 'C', 'D', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'X', 'Z', "\xc4\x8d", "\xc4\x8f", "\xc5\x88", "\xc5\x99", "\xc5\xa1", "\xc5\xa5", "\xc5\xbe", "\xc4\x8c", "\xc4\x8e", "\xc5\x87", "\xc5\x98", "\xc5\xa0", "\xc5\xa4", "\xc5\xbd");
    private $vowels = array('a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y', "\xc3\xa1", "\xc3\xa9", "\xc4\x9b", "\xc3\xad", "\xc3\xb3", "\xc3\xba", "\xc5\xaf", "\xc3\xbd", "\xc3\x81", "\xc3\x89", "\xc4\x9a", "\xc3\x8d", "\xc3\x93", "\xc3\x9a", "\xc5\xae", "\xc3\x9d");
    private $before_r = array('b', 'B', 'c', 'C', 'd', 'D', 'f', 'F', 'g', 'G', 'k', 'K', 'p', 'P', 'r', 'R', 't', 'T', 'v', 'V', "\xc4\x8d", "\xc4\x8c", "\xc4\x8f", "\xc4\x8e", "\xc5\x99", "\xc5\x98", "\xc5\xa5", "\xc5\xa4");
    private $before_l = array('b', 'B', 'c', 'C', 'd', 'D', 'f', 'F', 'g', 'G', 'k', 'K', 'l', 'L', 'p', 'P', 't', 'T', 'v', 'V', "\xc4\x8d", "\xc4\x8c", "\xc4\x8f", "\xc4\x8e", "\xc5\xa5", "\xc5\xa4");
    ...
    {
        return
            preg_replace_callback('#[^\ \n\t\x14\x15\x16\x{2013}\x{2014}\x{ad}-]{' . $this->wordLimit . ',}#u', array($this, 'pattern'), $text);
    }
    ...
    TexyModule
{
    ...
    __construct($texy)
    {
        ...
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\*)\*\*\*(?![\s*])(.+)' . TEXY_MODIFIER . '?(?<![\s*])\*\*\*(?!\*)' . TEXY_LINK . '??()#Uus', 'phrase/strong+em');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\*)\*\*(?![\s*])(.+)' . TEXY_MODIFIER . '?(?<![\s*])\*\*(?!\*)' . TEXY_LINK . '??()#Uus', 'phrase/strong');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<![/:])\/\/(?![\s/])(.+)' . TEXY_MODIFIER . '?(?<![\s/:])\/\/(?!\/)' . TEXY_LINK . '??()#Uus', 'phrase/em');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\*)\*(?![\s*])(\S+)' . TEXY_MODIFIER . '?(?<![\s*])\*(?!\*)' . TEXY_LINK . '??()#Uus', 'phrase/em-alt');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<![^\s.,;:<>()"\'' . TEXY_MARK . '-])\*(?![\s*])(.+)' . TEXY_MODIFIER . '?(?<![\s*])\*(?![^\s.,;:<>()"?!\'-])' . TEXY_LINK . '??()#Uus', 'phrase/em-alt2');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\+)\+\+(?![\s+])([^\r\n]+)' . TEXY_MODIFIER . '?(?<![\s+])\+\+(?!\+)()#Uu', 'phrase/ins');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<![<-])\-\-(?![\s>-])([^\r\n]+)' . TEXY_MODIFIER . '?(?<![\s<-])\-\-(?![>-])()#Uu', 'phrase/del');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\^)\^\^(?![\s^])([^\r\n]+)' . TEXY_MODIFIER . '?(?<![\s^])\^\^(?!\^)()#Uu', 'phrase/sup');
        $texy->registerLinePattern(array($this, 'patternSupSub'), '#(?<=[a-z0-9])\^([n0-9+-]{1,4}?)(?![a-z0-9])#Uui', 'phrase/sup-alt');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\_)\_\_(?![\s_])([^\r\n]+)' . TEXY_MODIFIER . '?(?<![\s_])\_\_(?!\_)()#Uu', 'phrase/sub');
        $texy->registerLinePattern(array($this, 'patternSupSub'), '#(?<=[a-z])\_([n0-9]{1,3})(?![a-z0-9])#Uui', 'phrase/sub-alt');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\")\"(?!\s)([^\"\r]+)' . TEXY_MODIFIER . '?(?<!\s)\"(?!\")' . TEXY_LINK . '??()#Uu', 'phrase/span');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\~)\~(?!\s)([^\~\r]+)' . TEXY_MODIFIER . '?(?<!\s)\~(?!\~)' . TEXY_LINK . '??()#Uu', 'phrase/span-alt');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\~)\~\~(?![\s~])([^\r\n]+)' . TEXY_MODIFIER . '?(?<![\s~])\~\~(?!\~)' . TEXY_LINK . '??()#Uu', 'phrase/cite');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\>)\>\>(?![\s>])([^\r\n]+)' . TEXY_MODIFIER . '?(?<![\s<])\<\<(?!\<)' . TEXY_LINK . '??()#Uu', 'phrase/quote');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<!\")\"(?!\s)([^\"\r\n]+)' . TEXY_MODIFIER . '?(?<!\s)\"(?!\")\(\((.+)\)\)()#Uu', 'phrase/acronym');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#(?<![' . TEXY_CHAR . '])([' . TEXY_CHAR . ']{2,})()\(\((.+)\)\)#Uu', 'phrase/acronym-alt');
        $texy->registerLinePattern(array($this, 'patternNoTexy'), '#(?<!\')\'\'(?![\s\'])([^' . TEXY_MARK . '\r\n]*)(?<![\s\'])\'\'(?!\')()#Uu', 'phrase/notexy');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#\`(\S[^' . TEXY_MARK . '\r\n]*)' . TEXY_MODIFIER . '?(?<!\s)\`' . TEXY_LINK . '??()#Uu', 'phrase/code');
        $texy->registerLinePattern(array($this, 'patternPhrase'), '#([' . TEXY_CHAR . '0-9@\#$%&.,_-]+)()(?=:\[)' . TEXY_LINK . '()#Uu', 'phrase/quicklink');
    ...
    TexyModule
{
    ...
    citeLink($link)
    {
        ...
        if ($link{0} === '[') {
            ...
            if ($ref) return
                Texy::prependRoot($ref->URL, $tx->linkModule->root);
        }
    ...
    TexyModule
{
    ...
    __construct($texy)
    {
        ...
        $texy->registerLinePattern(array($this, 'pattern'), '#\{\{([^' . TEXY_MARK . ']+)\}\}()#U', 'script');
    }
    ...
    pattern($parser, $matches)
    {
        ...
        if (preg_match('#^([a-z_][a-z0-9_-]*)\s*(?:\(([^()]*)\)|:(.*))$#iu', $cmd, $matches)) {
            ...
            if ($raw === '') $args = array(); else$args = preg_split('#\s*' . preg_quote($this->separator, '#') . '\s*#u', $raw);
        }
        if ($this->handler) {
            if (is_callable(array($this->handler, $cmd))) {
                array_unshift($args, $parser);
                return
                    call_user_func_array(array($this->handler, $cmd), $args);
            }
            if (is_callable($this->handler)) return
                call_user_func_array($this->handler, array($parser, $cmd, $args, $raw));
        }
...
TexyEmoticonModule
    ...
    TexyModule
{
    public $icons = array(':-)' => 'smile.gif', ':-(' => 'sad.gif', ';-)' => 'wink.gif', ':-D' => 'biggrin.gif', '8-O' => 'eek.gif', '8-)' => 'cool.gif', ':-?' => 'confused.gif', ':-x' => 'mad.gif', ':-P' => 'razz.gif', ':-|' => 'neutral.gif');
    public $class;
    public $root;
    public $fileRoot;
    ...
    beforeParse()
    {
        ...
        $pattern = array();
        foreach ($this->icons
                 as $key => $foo) $pattern[] = preg_quote($key, '#') . '+';
        $this->texy->registerLinePattern(array($this, 'pattern'), '#(?<=^|[\\x00-\\x20])(' . implode('|', $pattern) . ')#', 'emoticon', '#' . implode('|', $pattern) . '#');
    }
    ...
    TexyModule
{
    ...
    {
        $this->texy = $texy;
        $texy->registerBlockPattern(array($this, 'patternTable'), '#^(?:' . TEXY_MODIFIER_HV . '\n)?' . '\|.*()$#mU', 'table');
    }

    public
    function
    patternTable($parser, $matches)
    {
        if ($this->disableTables) return
            FALSE;
        list(, $mMod) = $matches;
        $tx = $this->texy;
        ...
        while (TRUE) {
            ...
            if ($parser->next('#^\|(.*)(?:|\|\ *' . TEXY_MODIFIER_HV . '?)()$#U', $matches)) {
                ...
                $rowClass = $rowCounter
                %
                2 === 0 ? $this->oddClass : $this->evenClass;
                ...
                $mContent = preg_replace('#(\[[^\]]*)\|#', "$1\x13", $mContent);
                foreach (explode('|', $mContent) as $cell) {
                    ...
                    if (isset($colModifier[$col])) $mod = clone$colModifier[$col]; else$mod = new
                    TexyModifier;
                    $mod->setProperties($mMod);
                    $elCell = new
    ...
    TexyModule
{
    ...
    postProcess($texy, &$s)
    {
        ...
        $this->baseDTD = $texy->dtd['div'][1] + $texy->dtd['html'][1] + $texy->dtd['body'][1] + array('html' => 1);
        $s = preg_replace_callback('#(.*)<(?:(!--.*--)|(/?)([a-z][a-z0-9._:-]*)(|[ \n].*)\s*(/?))>()#Uis', array($this, 'cb'), $s . '</end/>');
        foreach ($this->tagStack
                 as $item) $s .= $item['close'];
        $s = preg_replace("#[\t ]+(\n|\r|$)#", '$1', $s);
        $s = str_replace("\r\r", "\n", $s);
        $s = strtr($s, "\r", "\n");
        ...
        if ($this->lineWrap > 0) {
            $s = preg_replace_callback('#^(\t*)(.*)$#m', array($this, 'wrap'), $s);
        }
    ...
    cb($matches)
    {
        ...
        if ($mEnd) {
            ...
            $item = reset($this->tagStack);
            if ($item) $dtdContent = $item['dtdContent']; else$dtdContent = $this->baseDTD;
            ...
            if (!isset($this->texy->dtd[$mTag])) {
                ...
                if ($allowed && isset(TexyHtml::$prohibits[$mTag])) {
                    foreach (TexyHtml::$prohibits[$mTag] as $pTag) if (!empty($this->tagUsed[$pTag])) {
                        $allowed = FALSE;
                        break;
                    }
                }
    ...
    TexyObject
{
    ...
    const
        NONE = FALSE;
    const
        VERSION = TEXY_VERSION;
    const
        REVISION = '9338a11 released on 2010-05-20';
    const
        CONTENT_MARKUP = "\x17";
    const
    ...
    public $summary = array('images' => array(), 'links' => array(), 'preload' => array());
    public $styleSheet = '';
    public $alignClasses = array('left' => NULL, 'right' => NULL, 'center' => NULL, 'justify' => NULL, 'top' => NULL, 'middle' => NULL, 'bottom' => NULL);
    public $removeSoftHyphens = TRUE;
    public
    static $advertisingNotice = 'once';
    ...
    loadModules()
    {
        $this->scriptModule = new
        TexyScriptModule($this);
        $this->htmlModule = new
        TexyHtmlModule($this);
        $this->imageModule = new
        TexyImageModule($this);
        $this->phraseModule = new
        TexyPhraseModule($this);
        $this->linkModule = new
        TexyLinkModule($this);
        $this->emoticonModule = new
        TexyEmoticonModule($this);
        $this->paragraphModule = new
        TexyParagraphModule($this);
        $this->blockModule = new
        TexyBlockModule($this);
        $this->figureModule = new
        TexyFigureModule($this);
        $this->horizLineModule = new
        TexyHorizLineModule($this);
        $this->blockQuoteModule = new
        TexyBlockQuoteModule($this);
        $this->tableModule = new
        TexyTableModule($this);
        $this->headingModule = new
        TexyHeadingModule($this);
        $this->listModule = new
        TexyListModule($this);
        $this->typographyModule = new
        TexyTypographyModule($this);
        $this->longWordsModule = new
        TexyLongWordsModule($this);
        $this->htmlOutputModule = new
        TexyHtmlOutputModule($this);
    }
    ...
    registerLinePattern($handler, $pattern, $name, $againTest = NULL)
    {
        if (!is_callable($handler)) {
            ...
            throw
            new
            InvalidArgumentException("Handler '$textual' is not " . ($able ? 'callable.' : 'valid PHP callback.'));
        }
        if (!isset($this->allowed[$name])) $this->allowed[$name] = TRUE;
        $this->linePatterns[$name] = array('handler' => $handler, 'pattern' => $pattern, 'again' => $againTest);
    }
    ...
    registerBlockPattern($handler, $pattern, $name)
    {
        ...
        if (!isset($this->allowed[$name])) $this->allowed[$name] = TRUE;
        $this->blockPatterns[$name] = array('handler' => $handler, 'pattern' => $pattern . 'm');
    }
    ...
    process($text, $singleLine = FALSE)
    {
        if ($this->processing) {
            throw
            new
            InvalidStateException('Processing is in progress yet.');
        }
        $this->marks = array();
        $this->processing = TRUE;
        if (is_array($this->allowedClasses)) $this->_classes = array_flip($this->allowedClasses); else$this->_classes = $this->allowedClasses;
        if (is_array($this->allowedStyles)) $this->_styles = array_flip($this->allowedStyles); else$this->_styles = $this->allowedStyles;
        ...
        return
            TexyUtf::utf2html($html, $this->encoding);
    }
    ...
    processTypo($text)
    {
        ...
        return
            TexyUtf::utf2html($text, $this->encoding);
    }
    ...
    toText()
    {
        if (!$this->DOM) {
            throw
            new
            InvalidStateException('Call $texy->process() first.');
        }
        return
            TexyUtf::utfTo($this->DOM->toText($this), $this->encoding);
    }
    ...
    stringToText($s)
    {
        ...
        $s = preg_replace('#\n\s*\n\s*\n[\n\s]*\n#', "\n\n", $s);
        $s = self::unescapeHtml($s);
        $s = strtr($s, array("\xC2\xAD" => '', "\xC2\xA0" => ' '));
        return $s;
    }
    ...
    invokeAroundHandlers($event, $parser, $args)
    {
        if (!isset($this->handlers[$event])) return
            FALSE;
        $invocation = new
        TexyHandlerInvocation($this->handlers[$event], $parser, $args);
    ...
    checkURL($URL, $type)
    {
        if (!empty($this->urlSchemeFilters[$type]) && preg_match('#' . TEXY_URLSCHEME . '#A', $URL) && !preg_match($this->urlSchemeFilters[$type], $URL)) return
            FALSE;
        return
            TRUE;
    }
